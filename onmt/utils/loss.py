...
def build_loss_compute(model, tgt_field, opt, train=True):
    ...
    # elif "hyp" in opt.network_type:
    #     criterion = nn.CrossEntropyLoss(ignore_index=padding_idx, reduction='mean')
    else:
...
class NMTLossCompute(LossComputeBase):
    ...
    def __init__(self, criterion, generator, normalization="sents",
        ...
        # self.layernorm = torch.nn.LayerNorm(n_neg + 1).double()
    ...
    def orthogonal_penalty(self, _m, l_n_norm=2):
        # _m: h x n
        # I:  n x n
        _m = log_map_zero(_m.t()).t()
        m = torch.mm(torch.t(_m), _m)  # n x n
        # norm_m = _m.norm(p=2, dim=0)
        # mask = norm_m.unsqueeze(0) * norm_m.unsqueeze(1)
        # m = m / mask
    ...
    def _compute_semantic_coverage_loss(self, model, src_states, dec_states, tgtenc_states,
        ...
        pred = self.layernorm((0.0 - poinc_dist(batch_tgtenc_states, batch_src_states).squeeze(-1).reshape((-1, n_neg+1)))) # [n_sep, n_neg+1]
