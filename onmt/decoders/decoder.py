...
from onmt.models.stacked_rnn import StackedGRU_hype
...
class DecoderBase(nn.Module):
    ...
    def __init__(self, attentional=False):
...
class RNNDecoderBase(DecoderBase):
    ...
    def __init__(self, rnn_type, bidirectional_encoder, num_layers,
                 ...
                 target_encoder_type=None, detach_target_encoder=False, c=1.0, decision_type="eucl"
                 ):
        ...
        if context_gate is not None:
            self.context_gate = context_gate_factory(
                ...
                hidden_size, hidden_size, hidden_size, c=c
            )
        if "hyp" not in rnn_type:
            ...
            if not self.attentional:
                ...
                self.attn = GlobalAttention_hype(
                    hidden_size, coverage=coverage_attn,
                    attn_type=attn_type, attn_func=attn_func, c=c
                )
            ...
            if copy_attn and not reuse_copy_attn:
                if copy_attn_type == "none" or copy_attn_type is None:
                    raise ValueError(
                        "Cannot use copy_attn with copy_attn_type none")
                self.copy_attn = GlobalAttention_hype(
                    hidden_size, attn_type=copy_attn_type, attn_func=attn_func, c=c
                )
            else:
                self.copy_attn = None
        ...
        if target_encoder_type == 'rnn':
            self.target_encoder = self._build_rnn("GRU_hyper",
    ...
    def init_state(self, src, memory_bank, encoder_final):
        ...
        def _fix_enc_hidden(hidden):
            ...
            if 'ball' in self.__dict__['_modules'].keys():
                hidden = project_hyp_vec(hidden)
            return hidden
...
class HyperbolicRNNDecoder(RNNDecoderBase):
    ...
    def _run_forward_pass(self, tgt, memory_bank, memory_lengths=None):
        ...
        for emb_t in emb.split(1):
            if self.target_encoder is None:
                decoder_input = project_hyp_vec(torch.cat([emb_t.squeeze(0), input_feed], 1))
                ...
                decoder_input = project_hyp_vec(torch.cat([emb_t.squeeze(0), input_feed, tgtenc2dec], 1))
                rnn_output, dec_state = self.rnn(decoder_input, dec_state)
            ...
            if self.target_encoder is not None:
                if "tgtenc_states" not in attns:
                    attns["tgtenc_states"] = []
                attns["tgtenc_states"] += [tgt_enc_output]
        if not self.attentional and self.context_gate is None:
            if 'eucl' in self.decision_type:
                for i in range(len(dec_outs)):
                    dec_outs[i] = F.tanh(log_map_zero(dec_outs[i]))
                # dec_outs = [pmath.logmap0(dec_out, k=-1/self.ball.c) for dec_out in dec_outs]
            else:
                for i in range(len(dec_outs)):
                    dec_outs[i] = exp_map_zero(torch.tanh(self.layernorm(log_map_zero(dec_outs[i]))))
        return dec_state, dec_outs, attns

    def _build_rnn(self, rnn_type, input_size,
                   hidden_size, num_layers, dropout, c=1.0):
        assert("hyp" in rnn_type)
        stacked_cell = StackedGRU_hype
        return stacked_cell(num_layers, input_size, hidden_size, dropout, c)
