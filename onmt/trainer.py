            ...
            if valid_iter is not None and step % valid_steps == 0:
                if self.gpu_verbose_level > 0:
                    logger.info('GpuRank %d: validate step %d'
                                % (self.gpu_rank, step))
                # history_loss = pandas.DataFrame(self.history_loss)
                # history_loss.to_csv('/home/tianyu/Desktop/history_loss.csv')
                valid_stats = self.validate(
                    valid_iter, moving_average=self.moving_average)
                # self.history_valid_loss.append(valid_stats)
                # current_valid_acc = valid_stats.n_correct/valid_stats.n_words
        ...
        for k, batch in enumerate(true_batches):
            ...
            for j in range(0, target_size-1, trunc_size):
                ...
                try:
                    ...
                    # self.history_loss.append(batch_stats.loss)
                    self.report_manager.tensorboard_writer.add_scalar('loss', loss.item(), self.optim.training_step)
                ...
                #         if hasattr(torch.cuda,'empty_ccache'):
                #             torch.cuda.empty_cache()
                #     else:
                #         raise exception
