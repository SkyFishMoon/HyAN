...
class StackedGRU_hype(nn.Module):

    def __init__(self, num_layers, input_size, rnn_size, dropout, c):
        super(StackedGRU_hype, self).__init__()
        self.dropout = nn.Dropout(dropout)
        self.num_layers = num_layers
        self.layers = nn.ModuleList()

        for _ in range(num_layers):
            self.layers.append(MobiusGRUCell(input_size, rnn_size, c))
            input_size = rnn_size
