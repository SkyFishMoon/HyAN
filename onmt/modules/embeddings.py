...
import geoopt
from onmt.hyper.embeddings import Hyper_Embedding
...
class Embeddings(nn.Module):
    ...
    def __init__(self, word_vec_size,
        ...
        if pretrained:
            if "hyp" in network_type:
                self.manifold = manifold
                embeddings = [Hyper_Embedding.from_pretrained(project_hyp_vec(pretrained_embedding), freeze=False, padding_idx=pad, manifold=self.manifold)
                              for vocab, dim, pad in emb_params]
            else:
                pass
                # for pretrained euclidean embedding
        else:
            if "hyp" in network_type:
                self.manifold = manifold
                embeddings = [Hyper_Embedding(vocab, dim, padding_idx=pad, sparse=sparse, manifold=self.manifold)
                              for vocab, dim, pad in emb_params]
            else:
    ...
    def forward(self, source, step=None):
        ...
        if self.position_encoding:
            ...
            return pmath.project(source, k=-1 / self.manifold.c)
        else:
    ...
    #     #       in the client code.
    #     with torch.no_grad():
    #         self.make_embedding.emb_luts._modules['0'].weight.fill_(0)
