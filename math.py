...
def dis2plane_beta(
    ...
    *,
    ...
    return _dist2plane_beta(
        x, z, r, c, keepdim=keepdim, signed=signed, dim=dim
    )

# @torch.jit.script
def _dist2plane_beta(
    x: torch.Tensor,
    z: torch.Tensor,
    r: torch.Tensor,
    c: torch.Tensor,
    ...
    scale = 2 * z.norm(p=2, dim=0, keepdim=True) / torch.sqrt(c)
    cons = 2 * torch.sqrt(c)
    denom = 1 - c * x.norm(p=2, dim=-1, keepdim=True).pow(2)
    nom = 1 + c * x.norm(p=2, dim=-1, keepdim=True).pow(2)
    A = (cons * x.mm(z) / nom) * torch.cosh(cons * r).unsqueeze(0)
    B = (nom / denom) * torch.sinh(cons * r).unsqueeze(0)
    return scale * arsinh(A -B)
...
def _weighted_midpoint(
    ...
    #     reducedim.pop(dim)
    # gamma = _lambda_x(xs, k=k, dim=-1, keepdim=False).unsqueeze(1)
    # if weights is None:
    ...
    #     weights = weights.abs()
    # denominator = (gamma * weights).sum(-1, keepdim=True)
    # # nominator = (gamma * weights * xs).sum(reducedim, keepdim=True)
    # nominator = gamma * weights
    # temp = nominator/clamp_abs(denominator, 1e-10)
    # # two_mean = nominator / clamp_abs(denominator, 1e-10)
    # two_mean = torch.bmm(temp, xs)
    ...
    xs = 2 * xs / (1 + xs.norm(p=2, dim=-1, keepdim=True).pow(2))
    gamma = 1 / torch.sqrt(1 - xs.norm(p=2, dim=-1, keepdim=True).pow(2))
    if weights is None:
        weights = torch.tensor(1.0, dtype=xs.dtype, device=xs.device)
    else:
        weights = weights.unsqueeze(dim)
    if posweight and weights.lt(0).any():
        xs = torch.where(weights.lt(0), _antipode(xs, k=k, dim=dim), xs)
        weights = weights.abs()
    denominator = (gamma * weights).sum(reducedim, keepdim=True)
    nominator = (gamma * weights * xs).sum(reducedim, keepdim=True)
    # two_mean = nominator / clamp_abs(denominator, 1e-10)
    a_mean = nominator / clamp_abs(denominator, 1e-10)
    # a_mean = _mobius_scalar_mul(
    #     torch.tensor(0.5, dtype=xs.dtype, device=xs.device), two_mean, k=k, dim=dim
    # )
    if torch.any(k.gt(0)):
    ...
    a_mean = a_mean / (1 + torch.sqrt(1 - a_mean.norm(p=2, dim=-1, keepdim=True)))
    return a_mean
