...
def main(opt):
    ...
    elif opt.embedding_type == 'poincare':
        vocab = torch.load(opt.data + '_' + opt.embedding_type + '_' + 'glove' + '_' + opt.poincare_type + '_' + 'vocab.pt')
    elif opt.embedding_type == 'vanilla':
        vocab = torch.load(
            opt.data + '_' + opt.embedding_type + '_' + 'glove' + '_' + opt.poincare_type + '_' + 'vocab.pt')
    else:
...
if __name__ == "__main__":
    ...
    parser = _get_parser()
    parser.add_argument('--poincare_type', type=str,
                        required=True,
                        choices=['50x2D_cosh-dist-sq_init_trick', '50x2D_cosh-dist-sq',
                                 '100D_cosh-dist-sq_init_trick','100D_cosh-dist-sq',
                                 '100D_dist-sq_init_trick','100D_dist-sq',
                                 '100D_init_trick','100D'],
                        help='type of poincare word embedding')
    parser.add_argument('--dataset', type = str,
                        required=True,
                        choices = ['kp20k','kp20k_small','duc','inspec','krapivin','nus','semeval','stackexchange'])
    parser.add_argument('--embedding_type',type = str,
                        required=True,
                        choices=['euclidiean','poincare','vanilla'])
    parser.add_argument('--pretrained_wordembedding',type = int)
    parser.add_argument('--network_type', type = str,
                        required=True,
                        choices=['euclidiean', 'hyperbolic'])
    parser.add_argument('--c', type = float,
                        default=1.0)
    parser.add_argument('--wd', type=float, required=True)
    parser.add_argument('--decision_type', type=str, required=True, choices=['eucl', 'hyp'])
